...
#           BinMet = Binary Fraction times Metallicity
#           BinMetErr = Error in Binary Fraction times Metallicity

index=[]
for i in range(len(binfrac)):
    if ro[i]<10000 and lx_m_err[i]!=0 and metal[i]!=0 and binfrac[i]!=0:
        index.append(i)
...
BinMet = metal[index]*binfrac[index]
beta = metal[index]
beta_err = metal_err[index]
gamma = binfrac[index]
gamma_err = binfracerr[index]
BinMetErr = BinMet*np.sqrt((beta_err/beta)**2 +
                           (gamma_err/gamma)**2)

#%% Starting the emcee part

# Calling in the data
xerr, yerr = BinMetErr, LxMerr
x_obs, y_obs = BinMet, LxM

N = len(BinMet)
estimates = np.array([8.6,27.7])

params, error_int, samples = bayes_linear(x_obs,y_obs,xerr,yerr,p1=estimates,nWalkers=100,nBurn=200,nSample=4000)
...
plt.title('X-ray Luminosity per Unit Mass vs (Binary Fraction x Metallicity)')
...
#yn = np.zeros(100)
#plt.plot(xn,yn,'-k')
plt.xlabel(r'(Binary Fraction $\times$ Metallicity), $(\mathrm{binfrac}\times[Fe/H])$')
#plt.axis([min(x_obs+xerr),max(x_obs+xerr),-7, 7])
#plt.grid()
